# PUBLIC INTAKE V2 — RUNBOOK MIGRATION `202607290002`–`202607290006`

Áp dụng cho năm migration còn nợ tính đến 2026-07-30: `202607290002_full_pl3_editor.sql`,
...
> cùng số hiệu là lỗi chặn merge, nên **`005` giữ nguyên cho lazy Drive folder** (bản trên `main` là
> nguồn chính thức) và ghi chú nội bộ chuyển sang **`202607290006`**.
>
> Nếu một database nào đã ghi nhận `202607290005` trước đợt này, **phải kiểm lịch sử migration của
> chính database đó** trước khi push: xem mục 0.
...
- `docs/brain/04-current-tasks.md` ghi cả năm là **"chưa chạy"**.
- `docs/brain/06-ai-working-log.md` (mục "Phase 1 rehearsal reset") ghi một agent khác đã **reset
  ...
  `202607290005`/`06` vì hai file đó sinh ra sau) trên project ref `ddiaaweuqfvutogjckwc`, gọi project này là
...
- [ ] Chạy `npx tsx scripts/preflight-public-intake-v2-migrations.ts` **trước khi áp bất cứ gì** —
      script chỉ đọc, không ghi. Kết quả cho biết **chính xác** năm migration này đã áp hay chưa,
...
Cả năm đều **additive và idempotent** (`add column if not exists` / `drop column if exists` /
`create index if not exists` / `alter column drop not null`) — chạy lại trên schema đã có sẵn
...
```bash
supabase db dump --db-url "$SUPABASE_DATABASE_URL" -f preview-checkpoint-before-002-006.sql
```
...
where table_schema='public' and table_name='public_submissions' and column_name='drive_folder_id';
-- kỳ vọng: 1 dòng, is_nullable = 'YES'

select count(*) from information_schema.columns
where table_schema='public' and table_name='public_submissions'
  and column_name in ('drive_folder_state','drive_folder_lease_until','drive_folder_attempts');
-- kỳ vọng: 3

select conname from pg_constraint
where conname = 'public_submissions_drive_folder_state_check';
-- kỳ vọng: 1 dòng

select count(*) from public.public_submissions
where drive_folder_id is not null and drive_folder_state <> 'READY';
-- kỳ vọng: 0 (backfill đã đặt READY cho mọi hồ sơ đã có thư mục)
```
...
### Kiểm tổng hợp — một lệnh thay vì năm
...
- [ ] Báo PASS cho toàn bộ, bao gồm cả các migration cũ hơn (`202607280001`–`202607290001`) — script
      kiểm nguyên chuỗi phụ thuộc, không chỉ năm file này.
...
Rollback **code trước, migration sau**. Theo đúng thứ tự ngược (06→02):
...
-- 202607290005
-- ⚠️ CHỈ rollback được khi LAZY_DRIVE_FOLDER_CREATION_ENABLED = false VÀ không còn hồ sơ nào có
-- drive_folder_id NULL. Đặt lại NOT NULL trong lúc còn hồ sơ chưa có thư mục sẽ làm lệnh lỗi —
-- đúng như vậy, vì ép NOT NULL bằng cách xóa/điền bừa drive_folder_id là mất đường tìm lại ảnh.
alter table public.public_submissions
  drop constraint if exists public_submissions_drive_folder_state_check;
drop index if exists public.public_submissions_drive_folder_lease_idx;
alter table public.public_submissions
  drop column if exists drive_folder_attempts,
  drop column if exists drive_folder_lease_until,
  drop column if exists drive_folder_state;
alter table public.public_submissions
...
trạng thái/lease tạo thư mục Drive (`drive_folder_*` — chỉ là dữ liệu điều phối, tạo lại được).
...
Các mảng code liên quan tới năm migration này (xem `CHATGPT_HANDOFF.md`):

- Bàn làm việc PL3 đầy đủ — cần `202607290002`.
- Tối ưu hàng đợi (Phase 1) — cần `202607290004`.
- Lazy Drive folder (Phase 3, cờ mặc định tắt) — cần `202607290005`.
- Ghi chú nội bộ (Đợt 2A-2) — cần `202607290006`. **Đây là migration chặn deploy mạnh nhất trong
  năm:** `internal_notes` nằm trong `SUBMISSION_SELECT` dùng chung, nên deploy code trước migration
  làm mọi truy vấn đọc hồ sơ lỗi chứ không chỉ hỏng chức năng ghi chú.
- `202607290003` không gắn với tính năng mới; là dọn cột thừa, chạy được độc lập bất cứ lúc nào sau
  `202607290002`.

Checklist deploy:

- [ ] Năm migration đã chạy trên preview, theo đúng thứ tự.
