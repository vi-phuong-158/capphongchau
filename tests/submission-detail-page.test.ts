 ...
 *
 *   - `loadSubmissionDetail` trả `null` = **hồ sơ không tồn tại** (hoặc ngoài quyền đọc) → phải
 *     `notFound()`, tuyệt đối không render khung trang rỗng rồi để client tự dò.
 *   - `loadSubmissionDetail` **ném lỗi** = sự cố tạm (mất kết nối cơ sở dữ liệu) → phải render với
...
const loadSubmissionDetail = vi.fn();
...
vi.mock("@/modules/submissions/detail-view", () => ({ loadSubmissionDetail }));
...
type DetailProps = {
  ...
  initialSubmission: SubmissionDetailView | null;
};
...
describe("trang duyệt hồ sơ — nhánh không tìm thấy so với nhánh lỗi tạm", () => {
  it("hồ sơ không tồn tại thì trả 404, không render trang rỗng", async () => {
    loadSubmissionDetail.mockResolvedValue(null);
  ...
  it("lỗi tạm khi nạp sẵn KHÔNG thành 404 — vẫn render để client tự fetch", async () => {
    loadSubmissionDetail.mockRejectedValue(new Error("kết nối cơ sở dữ liệu bị ngắt"));
  ...
  it("nạp sẵn thành công thì truyền thẳng hồ sơ xuống component, không gọi lại API", async () => {
    const submission = { id: "1a2b3c" } as unknown as SubmissionDetailView;
    loadSubmissionDetail.mockResolvedValue(submission);
  ...
  /**
   * Audit `SUBMISSION_SENSITIVE_DETAIL_VIEWED` nằm trong `loadSubmissionDetail`. Nếu trang tự dựng
   ...
   */
  it("luôn đi qua loadSubmissionDetail để giữ dấu vết audit", async () => {
    loadSubmissionDetail.mockResolvedValue({ id: "9z" } as unknown as SubmissionDetailView);

    await renderPage("9z");

    expect(loadSubmissionDetail).toHaveBeenCalledWith("9z", OFFICER, expect.any(String));
  });
  ...
  it("người không đủ quyền bị đưa về /profile chứ không thấy hồ sơ", async () => {
    ...
    expect(loadSubmissionDetail).not.toHaveBeenCalled();
  });
  ...
  it("phân quyền admin được suy ra từ vai trò", async () => {
    loadSubmissionDetail.mockResolvedValue({ id: "1a2b3c" } as unknown as SubmissionDetailView);
