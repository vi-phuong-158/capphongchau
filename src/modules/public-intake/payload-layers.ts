 ...
 * officialPayload (bản đã tiếp nhận) > workingPayload (cán bộ đang duyệt/sửa) > citizenPayload (bản người dân gửi) > draft (nháp cũ)
 */
export function effectivePayload(record: SubmissionRecord): IntakeDraft | null {
  return record.officialPayload ?? record.workingPayload ?? record.citizenPayload ?? record.draft;
}
...
export function payloadLayerOf(record: SubmissionRecord): PayloadLayer | "DRAFT" {
  if (record.officialPayload) return "OFFICIAL";
