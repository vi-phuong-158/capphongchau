...
```json
{
  ...
  "files": [
    {
      "fileId": "uuid",
      "localRelativePath": "files/AA 09489120-GCN-01.jpg",
...
```ts
export function compareCitizenAndAi(
  citizen: IntakeDraft,
  ai: NormalizedExtraction,
...
2. Magic bytes `PK`, kích thước > 0. 5. Mở workbook bằng ExcelJS: 3 sheet `PL3` / `Ton dong` / `Canh bao`.
3. Fixture 1 GCN 20 thửa → đúng 20 dòng. 7. Fixture 2.500 hồ sơ → không cắt, `x-export-truncated = 0`.
4. Unicode tiếng Việt nguyên vẹn. 9. Ô rỗng là chuỗi rỗng. 10. Không dữ liệu → file chỉ có header, không hỏng.
