...
- Comment ngay trong file nói thẳng: *"Tệp PDF đổi tên thành `.jpg` vẫn bị chặn ở đó"* — [image-format.ts:15-16](src/modules/public-intake/image-format.ts:15).
...
**Sự thật:** repo có đúng **một** cột `draft_json` ([schema.sql:64](supabase/migrations/202607230001_supabase_schema.sql:64)), bị người dân ghi lúc nháp và cán bộ ghi đè lúc sửa. Không có `citizen_payload`, không có `working_payload`, không có `official_payload`. Chính câu tiếp theo trong `GEMINI.md` — *"Không được tái sử dụng một cột JSON rồi ghi đè tuần tự làm mất lịch sử"* — mô tả **chính xác** cái repo đang làm.
...
`GEMINI.md` §5.2 đề xuất `**/inbox/*`, `**/processing/*`, `**/completed/*`… — quá rộng. Nếu sau này có `src/modules/.../inbox/`, Git sẽ bỏ qua source thật. Chính `GEMINI.md` cũng cảnh báo *"Không dùng quy tắc quá rộng làm bỏ qua source trong `agent/`"* rồi lại đề xuất đúng quy tắc quá rộng.
