...
function prepareSubmissionsSheet_(spreadsheet) {
  ...
  defaultSheet
    .getRange(1, 1, 1, SUBMISSION_HEADERS.length)
    .setFontWeight("bold")
  ...
  defaultSheet
    .getRange(2, SUB_COL.STARTED_AT, defaultSheet.getMaxRows() - 1, 2)
    .setNumberFormat("dd/MM/yyyy HH:mm:ss");
  defaultSheet.getRange(2, SUB_COL.PHONE, defaultSheet.getMaxRows() - 1, 2).setNumberFormat("@");
...
function prepareFilesSheet_(spreadsheet) {
  ...
  sheet
    .getRange(1, 1, 1, FILE_HEADERS.length)
    .setFontWeight("bold")
...
function prepareGuideSheet_(spreadsheet) {
  ...
  const rows = [
    ...
    [
      "QUY TRÌNH",
      "Người dân gửi hồ sơ → cán bộ kiểm tra thư mục → nhập vào App chính → cập nhật trạng thái đồng bộ.",
    ],
